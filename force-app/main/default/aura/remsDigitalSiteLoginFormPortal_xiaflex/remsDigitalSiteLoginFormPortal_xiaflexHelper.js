({
    ...
    handleLogin: function (component, event, helpler) {
        ...
        if(component.get("v.programType") == 'MDP'){
            var MDP = 'MDP';
            persona = persona+MDP;
        }
        var username = component.find("username").get("v.value");
        var password = component.find("password").get("v.value");
		if(persona != '' && username!= '' && password!= ''){
        var action = component.get("c.loginXiaflex");
        var startUrl = component.get("v.startUrl");
        var programName = component.get("v.programName");
        startUrl = decodeURIComponent(startUrl);
        action.setParams({username:username, password:password, startUrl:startUrl, programName:programName, persona:persona});
        ...
        $A.enqueueAction(action);
             ...
             var RequiredFieldError = $A.get("$Label.c.Required_Field_Error_Message");
             component.set("v.errorMessage",RequiredFieldError);
                component.set("v.showError",true);
        }
    },
