({
	setColumns : function(component, event, helper) {
        var cols = [
            ...
            {
                label: 'Should Patient be Monitored?',
    ...
    setRows : function(component, event, helper) {
        ...
        var action = component.get("c.getRMRAndCOICases"); 
        action.setParams({ patientAccountId : recId });
            ...
            if (state === "SUCCESS") {
                ...
                for (var i = 0; i < returnResponseList.length; i++ ) {
                    ...
                    if (currentRecord.RecordType && currentRecord.RecordType.Name) {
                        ...
                        record.activityType = record.activityType == 'Patient COI' ? 'Change of Information' : record.activityType;
                    }
                    record.patientMonitored = currentRecord.SYN_Should_patient_be_monitored__c;
