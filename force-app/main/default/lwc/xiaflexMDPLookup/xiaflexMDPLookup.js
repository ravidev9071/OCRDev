...
export default class XiaflexMDPLookup extends LightningElement {
    @track records = [];
    @track name = '';
    @track inputVal ='';
    ...
    handleSearchKeyword() {
        ...
        if(!this.isHCP && !this.isHCS) {
            ...
            searchHCPRecords({ searchKeyWord: this.inputVal, programName : 'XIAFLEX', programType : 'MDP' })
                .then(result => {
                    ...
                    this.prescriber_records = result.map((obj) => {
                        return {
                            ...
                            LastName: obj.US_WSREMS__Prescriber__r.LastName ? obj.US_WSREMS__Prescriber__r.LastName : ''
                        };
                    });
        ...
        searchRecords({ inputValue: this.inputVal, participantType: this.participantType,searchType : 'Lookup', programType : 'MDP' })
            .then(result => {
                this.showError = false;
                this.showTable = true;
                this.records = [...result];
                console.log('test'+ JSON.stringify(this.records));
