...
import getAccountCreationPage from "@salesforce/apex/OCRFormCtrl.getAccountCreationPage";
import createAccount from "@salesforce/apex/OCRFormCtrl.createAccount";
import getStakeholderFieldConfigs from '@salesforce/apex/OCRFormCtrl.getStakeholderFieldConfigs';
...
import createSubstakeHolderCase from "@salesforce/apex/OCRRemsAccountCreation.createSubstakeHolderCase";
...
export default class StakeHolderAccountCreationCmp extends NavigationMixin(LightningElement) {
  ...
  @track AccountCreationMetaData = [];
  @wire (getAccountfieldMapping) 
  duplicateConfigMetadata({error,data}){
    if(data){
      data.forEach(config =>{
        let cnf = {...config};
        Object.keys(cnf).forEach(key =>{
          if(key.indexOf('Title__c') > -1){
            cnf['Title__c'] = config[key];
          }
        })
         
        this.AccountCreationMetaData.push(cnf)
      })
    }
  }
...
@api showPopup = false;
 ...
 handleGetPermissionSets(){
    ...
    if (result) {
      ...
      try {
        ...
        if (this.isInboundOutboundCall) {
          ...
          if (sameasParticipantField != null) {
        ...
        if(this.serviceConfigRecord[`${nameSpace}Service_Record_Type__c`] == 'ReCertification' &&  this.serviceConfigRecord[`${nameSpace}Participant_Type__c`] == "Health Care Setting"){
          var kafldNameKey = `${nameSpace}` + 'Knowledge_Assessment__c';
          let kafield = this.template.querySelector(
            `lightning-input-field[data-field="${kafldNameKey}"]`
          );
          kafield.value = 'Not Started';
        }
        if(this.serviceConfigRecord[`${nameSpace}Validate_AR_Info__c`]) {
            let arInfo = await primaryARDetails({
              ...
              participantType: this.serviceConfigRecord[`${nameSpace}Participant_Type__c`]
            });
            if(arInfo){
      ...
      getServiceCreation({serviceConfigObj: this.serviceConfigRecord,documentCheckListId:this.documentCheckListId
        ...
        .then((result) => {
          ...
          if (currentUrl.includes('DocumentReview') || this.serviceFromAccount) {
            if ((this.serviceFields[0].value[1].fieldName) && this.serviceFields[0].value[1].fieldName === `${nameSpace}Channel__c`) {
  ...
  async isDuplicateHoldersAvailable (fields){
    ...
    this.generateDupTablesData(this.currentDuplicateConfigs, this.duplicateAccounts);
    this.OpenModelPopForDuplicateAccounts(this.duplicateAccounts);
  }
  @track tablesData = [];
  generateDupTablesData(configs,duplicateAccounts){
    this.tablesData = [];
    let nameSpace = this.namespaceVar;
    let tables = {};
    Object.keys(duplicateAccounts).forEach(stakeholder=>{
      if(duplicateAccounts[stakeholder].length > 0){
        tables[stakeholder] = {data : duplicateAccounts[stakeholder],name:stakeholder,show:true};
      }else{
        tables[stakeholder] = {show:false,data:[]};
      }
    })
    ...
    configs.forEach(config=>{
      ...
      if(config[nameSpace+'Main_Stakeholder_Account__c']){
        name = 'mainstake'
      }else if(!config[nameSpace+'Main_Stakeholder_Account__c']){
        name = config[nameSpace+'Source_Reference_Field__c'];
      }
      if(name){
  ...
  get tables(){
    return Object.values(this.tablesData);
  }

  OpenModelPopForDuplicateAccounts(data){
    this.tables.forEach(stakeholder =>{
      if(stakeholder.show){
        this.isOpen = true;
      }
    })
    if(!this.isOpen){
      ...
      if(substakeConfig.length > 0){
        substakeConfig.forEach(substakeC =>{
          this.createStakeHolder.push(substakeC[`${this.namespaceVar}Source_Reference_Field__c`]);
        })
      }
    }
  }
    ...
    async hideModal(){
      ...
      Object.keys(this.tablesData).forEach(table =>{
        if (table === 'mainstake') {
          this.accountId = '';
        }else if (table.name !== 'mainstake') {
          if(this.serviceType === 'Knowledge Assessment') {
            this.accountId = '';
          }
          this.createStakeHolder.push(table);
          isSubStakeNew = true;
        }
      })
      ...
      if (result) {
        this.skipDupcheck = true;
        this.ShowSpinner = false;
        let npiField = this.template.querySelector(
          `lightning-input-field[data-field="${this.namespaceVar+'NPI__c'}"]`
        );
        let serviceConfig = typeof this.serviceConfig == 'object'?this.serviceConfig: this.serviceConfigRecord;
        let recordType = serviceConfig[this.namespaceVar+'Requestor_Type__c'];
        if(npiField)
          this.handleDuplicateNPI(npiField.value,recordType,this.programId);
    ...
    async save() {
      ...
      if (!this.selectedDuplicateRecord) {
        ...
        if (this.callingFrom === 'NewAccount') {
          ...
          Object.keys(this.tablesData).forEach( async type => {
            if(selectedDupAccountTypes.indexOf(type) === -1 && this.tablesData[type]?.data?.length == 0){
              if(type !== 'mainstake'){
                isSubStakeNew = true;
              }
            }else if (selectedDupAccountTypes.indexOf(type) === -1 && this.tablesData[type]?.data?.length > 0) {
              showConfirm = true;
              confirmLabels.push(this.tablesData[type].config[nameSpace+'Title__c']);
              if (type === 'mainstake') {
                this.accountId = '';
                isMainStakeNew = true;
              } else {
            ...
            } else if (type === 'mainstake' && this.tablesData[type]?.data?.length > 0) {
            ...
            } else if(this.tablesData[type]?.data?.length > 0) {
              if(this.serviceType === 'Knowledge Assessment') {
                this.accountId = this.selectedDuplicateRecord[type].Id;
                await this.getCases()
                if (this.isCaseOpen) {
          ...
          if((showConfirm && isConfirmed) || this.skipDupcheck){
            let keys = Object.keys(this.tablesData);
            for(let i=0; i<keys.length; i++){
              let type = keys[i];
            //.forEach( async type => {
              if (type === 'mainstake' && this.tablesData[type]?.data?.length > 0 && !isMainStakeNew && !this.isCaseOpen) {
              ...
              }else if(this.tablesData[type]?.data?.length > 0 && type !== 'mainstake') {
                ...
                if(!isSubStakeNew)
                  isSubStakeNew = false;
                let refFieldName = nameSpace + type;
                let inputField = this.template.querySelector(
                    `lightning-input-field[data-field="${refFieldName}"]`
                );
                if(!this.selectedDuplicateRecord[type]){
                  this.createStakeHolder.push(type);
                }
                if (this.selectedDuplicateRecord[type] && inputField.fieldName === refFieldName) {
                  inputField.value = this.selectedDuplicateRecord[type].Id;
                  this.substakeHolderUpdateList[inputField.fieldName]=inputField.value;
                  await this.handleLookUpValues(refFieldName, inputField.value, '');
                }
  ...
  async handleSubmit(event) {
    ...
    console.log('recordTypeData+ '+ this.recordTypeData);
    ...
    if(fields['Status'] === 'Complete'){
      ...
      if(this.serviceConfigRecord[`${nameSpace}Service_Record_Type__c`] == 'Change_Authorized_Representative'){
        if(this.isNullOrEmpty(this.accountId)){
          this.accountId = fields[`${nameSpace}Healthcare_Setting__c`] || fields[`${nameSpace}Pharmacy__c`] || fields[`${nameSpace}Distributor__c`]
        }
        if(!this.isNullOrEmpty(this.accountId)){
          await this.getCases();
        }
        if(this.isCaseOpen) {
          let message = "Account already exist with open Change AR case";
          this.showToastNotification(
              "Error",
              message,
              "Error"
        ...
        fields[`${nameSpace}Email_ar__c`] = this.primaryAREmail;
        let noKA = ['Inpatient Pharmacy','Distributor']
        let allowedStatus = ['Certified']
        if(!allowedStatus.includes(this.mainStakeHolderStatus)){
          this.ShowSpinner = false;
          ...
          fields['Status'] = '';
          if(this.isNullOrEmpty(fields[`${nameSpace}Authorized_Representative_New_Primary_AR__c`] ) && !this.isNullOrEmpty(fields[`${nameSpace}AR_New_Secondary_AR__c`] )){
            fields['Status'] = 'Complete';
            fields[`${nameSpace}Outcome__c`] = 'Complete';
          }else if(fields[`${nameSpace}AR_Signature_New_Primary_AR__c`] == 'NO' || fields[`${nameSpace}KA_New_Primary_AR__c`] != 'Passed'){
            fields['Status'] = 'Draft';
    ...
    if(!this.validFieldValues && this.isInputValid()){
      ...
      if(this.serviceConfigRecord[`${nameSpace}Service_Record_Type__c`] == 'ReCertification' ){
        this.skipDupcheck = this.serviceConfigRecord[`${nameSpace}Validate_Dupcheck__c`];
      }

      if(!this.serviceConfigRecord[`${nameSpace}Validate_Dupcheck__c`] && !this.skipDupcheck && !this.isInboundOutboundCall){
    ...
    if (!this.validFieldValues && this.isInputValid() && !this.checkNPIDuplicateFound && !this.isOpen && this.skipDupcheck) {
      ...
      if(this.byPassAccountSelection && !this.isOpen && `${nameSpace}Case_Record_Type__c` in this.serviceConfigRecord && !this.isKAPrescriber && !this.isInboundOutboundCall){
        ...
        let configByRefField = {};
        configs.forEach(config =>{
          if(!config[nameSpace+'Main_Stakeholder_Account__c']){
            configByRefField[nameSpace+config[nameSpace+'Source_Reference_Field__c']] = config;
            ...
            targetFields.forEach(targetField =>{
              ...
              if(fields[val] && this.createStakeHolder.indexOf(sourceReferenceField) !== -1){
        ...
        await createSubStakeHolderAccounts({fieldMapValue: this.storeAccountFieldValues, RequestRecordType: this.serviceConfigRecord[`${nameSpace}Case_Record_Type__c`] , programId: this.programId, programName: this.programName , channel : channel})
          .then(async  (result) => {
            console.log('createSubStakeHolderAccounts ',result);
            if (result) {
              for(const key in result){
                ...
                await createSubstakeHolderCase({accountId:result[key], config:configByRefField[array[0]]})
                .then(response =>{

                }).catch(errors =>{
                  this.showToastNotification(
                      "Error",
                      reduceErrors(errors).join(", "),
                      "Error"
                  );
                  this.stopCreation = true;
                  this.ShowSpinner = false;
      ...
      this.fields = fields;
      let fieldsToSubmit = {};
      if(this.participantType && this.serviceType && (this.accountId == undefined || this.accountId == '')){
        let configs = this.allMatchingDuplicateConfigs();
        let mainstakeConfig;
        configs.forEach(config =>{
          if(config[nameSpace+'Main_Stakeholder_Account__c']){
            mainstakeConfig = config;
          }
        });
        let mappings;
        if(mainstakeConfig){
        mappings = await getStakeholderFieldConfigs({
          programName: mainstakeConfig[`${nameSpace}Program__r`].MasterLabel,
          sourceRTName: mainstakeConfig[`${nameSpace}Case_RecordTypeName__c`]
        })
      }
        ...
        if(mappings){
          mappings.forEach(mapping =>{
            ...
            keys.forEach(item =>{
              if(item.indexOf('Source_Field__c') > -1 && fields[mapping[item]]){
                fieldsToSubmit[mapping[item]] = fields[mapping[item]];
              }
            })
          })
        }
      ...
      console.log('fieldsToSubmit: '+JSON.stringify(fieldsToSubmit));
  ...
  handleFieldVisibility(fieldName, fieldValues, event) {
    ...
    this.AccountCreationMetaData.forEach(config =>{
     
      if(config!=undefined  && (config[`${nameSpace}Program__r`].MasterLabel).toLowerCase() === (this.programName).toLowerCase()){
        let inputField = this.template.querySelector(
            `lightning-input-field[data-field="${nameSpace}${config[`${nameSpace}Source_Reference_Field__c`]}"]`
        );
        if(!config[nameSpace+'Main_Stakeholder_Account__c'] &&
  ...
  handleInputChange(event) {
    ...
    if(this.serviceType == 'Enrollment' || this.callingFrom === 'NewAccount' || this.serviceType === 'Knowledge Assessment'){
      ...
      console.log('allDuplicateConfigs ',JSON.stringify(allDuplicateConfigs));
  ...
  async handleLookUpValues(fieldName,fieldValue,event){
     ...
     console.log('Fields:: ',fieldName);
      try {
        ...
        console.log('getFieldValues ',JSON.stringify(getFieldValues));
  ...
  asyncUpdateFieldValues(event,parent,parentName) {
    ...
    for (const [key, value] of this.mapData.entries()) {
            if(this.serviceConfigRecord[`${nameSpace}Service_Record_Type__c`] == 'Change_Authorized_Representative' && (parentName == `${nameSpace}Healthcare_Setting__c` || parentName == `${nameSpace}Pharmacy__c` || parentName == `${nameSpace}Distributor__c`) && key == `${nameSpace}Email_ar__c`){
              this.primaryAREmail = value;
            }
  ...
  createServiceAccount = async () => {
    ...
    if (result) {
      console.log('result After Account :'+JSON.stringify(result));
        ...
        if (result) {
         ...
         if(this.contactid){
          this[NavigationMixin.Navigate]({
            ...
            attributes: {
              componentName: this.namespaceVar+"newCaseFormForRedirection"
            },
            state: {
    ...
    if (result) {
      ...
      if (this.accountId && !this.secondCaseId) {
        let fields = {
          ...
          [this.serviceConfigRecord[`${nameSpace}Participant_Field__c`]]: this.accountId
        };
  ...
  asyncUpdatePrimaryFieldValues(event) {
      ...
      if(fieldMap.has(ele.fieldName) && fieldMap.get(ele.fieldName)) {
        switch (ele.name) {
          ...
          case 'REFERENCE':
            ...
            break;
          default:
            if(this.serviceConfigRecord[`${nameSpace}Service_Record_Type__c`] == 'Change_Authorized_Representative' && ele.fieldName == `Status`){
              ele.value = 'Draft';
              this.mainStakeHolderStatus =fieldMap.get(ele.fieldName);
            }else{
    ...
    this.serviceFields.forEach((elements) => {
