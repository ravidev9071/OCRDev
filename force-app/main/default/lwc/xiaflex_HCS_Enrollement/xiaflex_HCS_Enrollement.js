...
export default class Xiaflex_HCS_Enrollement extends NavigationMixin(LightningElement) {
    ...
    async handleContinue(event) {
        var val = this.template.querySelectorAll(".continueInput");
        var isValid = true;
        var emailVal, confirmEmailVal;
        val.forEach(rec => {
            ...
            if (rec.value && rec.value.length > 0) {
                if(rec.name == 'email'){
                    emailVal = rec.value;
                    this.arRecord[rec.name] = rec.value;
                } else if(rec.name == 'confirmEmail'){
                    confirmEmailVal = rec.value;
                } else{
                    this.arRecord[rec.name] = rec.value;
                }
            }
                            ...
                            insertHcsEnrollment({'inputRecords': inputRecords, 'hcpRecord': JSON.stringify(this.hcpRecordList)}).then((result) => {
                                ...
                                if(result == 'dupFound'){
                                    ...
                                    this[NavigationMixin.Navigate]({
                                        ...
                                        attributes: {
                                        name: 'pharmHCS_Enrollment_Form_Success__c'
                                    }
                                    });
