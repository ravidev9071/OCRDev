...
export default class Xiaflex_AR_pharmHCSEnrollmentForm extends NavigationMixin(LightningElement) {
    ...
    getArRecord(){
        ...
        .then(result => {
                    if (result != null && result != undefined) {
                        ...
                        this.arRecord.phoneType = result.US_WSREMS__Phone_Type__c;
    ...
    async handleHcpAccountRecords(currentIndex){
        ...
        if (accRecords != null) {
            var hcpAccount = accRecords.map((acc, index) => ({ 
                ...
                Address: (acc.US_WSREMS__Health_Care_Setting__c && acc.US_WSREMS__Health_Care_Setting__r) ? (acc.US_WSREMS__Health_Care_Setting__r.US_WSREMS__Address_Line_1__c ? acc.US_WSREMS__Health_Care_Setting__r.US_WSREMS__Address_Line_1__c : null) : null,
                City: (acc.US_WSREMS__Health_Care_Setting__c && acc.US_WSREMS__Health_Care_Setting__r) ? (acc.US_WSREMS__Health_Care_Setting__r.US_WSREMS__City__c ? acc.US_WSREMS__Health_Care_Setting__r.US_WSREMS__City__c : null) : null,
                State: (acc.US_WSREMS__Health_Care_Setting__c && acc.US_WSREMS__Health_Care_Setting__r) ? (acc.US_WSREMS__Health_Care_Setting__r.US_WSREMS__State__c ? acc.US_WSREMS__Health_Care_Setting__r.US_WSREMS__State__c : null) : null,
                ZIPCode: (acc.US_WSREMS__Health_Care_Setting__c && acc.US_WSREMS__Health_Care_Setting__r) ? (acc.US_WSREMS__Health_Care_Setting__r.US_WSREMS__Zip__c ? acc.US_WSREMS__Health_Care_Setting__r.US_WSREMS__Zip__c : null) : null,
                Phone: acc.US_WSREMS__Prescriber__r.Phone ? acc.US_WSREMS__Prescriber__r.Phone : null,
    ...
    handleChange(event) {
        ...
        if (fieldName === 'fax' || fieldName === 'phone') {
            ...
            this.arRecord[event.target.name] = fieldValue;
        } else {
            this.arRecord[event.target.name] = fieldValue;
        }
    }
    ...
    validateInputs(event) {
        ...
        inputFields.forEach(inputField => {
            ...
            if(fieldName === 'fax' || fieldName === 'phone' ||  fieldName === 'hcsphone' ||  fieldName === 'hcsfax') {                
                fieldValue = this.formatPhoneNumber(numericPhone);
    ...
    handleSubmit() {
        ...
        let isInputsCorrect1 = [...this.template.querySelectorAll('.req')]
        .reduce((validSoFar, inputField) => {
            inputField.setCustomValidity('');
            if(!inputField.value) {
        ...
        isInputsCorrect = isInputsCorrect1;
